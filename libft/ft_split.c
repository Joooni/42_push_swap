...
/*   Updated: 2022/03/25 10:01:49 by jsubel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

size_t	ft_split_count_words(const char *s, char c)
{
	size_t	i;
	size_t	marker;
	size_t	nbr_of_words;
...
char	**ft_split(const char *s, char c)
{
	...
	i = 0;
	j = 0;
	lst = (char **)malloc(sizeof(char *) * (ft_split_count_words(s, c) + 1));
	if (!lst)
		return (NULL);
	while (i < ft_split_count_words(s, c))
	{
		while (s[j] == c && s[j] != '\0')
			j++;
